...
object options {
  ...
  class Subcommand[T](
      ...
      private[options] val subparser: Option[OParser[_, Any]]
  ) {
  ...
  object Subcommand {
    def empty(transform: OParser[Unit, Any] => OParser[Unit, Any] = identity) =
      new Subcommand((), transform, None)
  ...
  private inline def makeParserRec[ArgsT <: Tuple, Labels, Args <: Product](
      ...
      position: Int
  ): Option[scopt.OParser[_, Args]] =

    def getCurrent = instance.productElement(position)

    inline (erasedValue[ArgsT], erasedValue[Labels]) match
      case (EmptyTuple, EmptyTuple) => acc.asInstanceOf[Option[scopt.OParser[_, Args]]]
      case (_: (Subcommand[τ] *: rargs), (_: (label *: rlabels))) =>
        ...
        val end = subvalue.transform {
          val act = builder.cmd(name).action { (_, args) =>
            val inst = subvalue
            inst.isUsed = true
            args
          }
          subvalue.subparser.fold(act)(x => act.children(x))
        }
