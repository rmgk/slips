...
class OptionsTest extends munit.FunSuite {
  ...
  case class CliArgs(
      ...
      flagish: Argument[Boolean, Single, Style.Named] = Argument(_.text("test simple args"), Some(false))
  )
  ...
  test("basic") {
    ...
    val res = scopt.OParser.parse(p, Seq("42", "--second", "/a/b", "32"), instance)
    ...
    assertEquals(res.get.flagish.value, false)
  }
